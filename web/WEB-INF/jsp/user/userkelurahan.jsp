...
<script type="text/javascript">
    ...
    function newUser() {
        ...
        $('#fm').form('clear');
        url = '${pageContext.request.contextPath}/user/userkelurahan/tambah';
    }
    function editUser() {
        ...
        if (row) {
            ...
            $('#fms').form('load', row);
            url = '${pageContext.request.contextPath}/user/userkelurahan/edit';
        }
    }
    ...
    function removeUser() {
        var row = $('#dg').datagrid('getSelected');
        if (row) {
            $.messager.confirm('Konfirmasi', 'Apakah anda yakin akan menghapus user kelurahan tersebut?', function(r) {
                if (r) {
                    $.post('${pageContext.request.contextPath}/user/userkelurahan/hapus', {id: row.username}, function(result) {
                        if (result.success) {
                            $('#dg').datagrid('reload');	// reload the user data
                        } else {
    ...
    <div class="manajemendata">
        ...
        <table id="dg" class="easyui-datagrid" 
               url="${pageContext.request.contextPath}/user/userkelurahan/tampil"
               toolbar="#tb"
...
<div id="dlg" class="easyui-dialog" style="width:400px;height:310px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">
    <div class="ftitle">Informasi User HelpDesk</div>
...
<div id="dlgedit" class="easyui-dialog" style="width:400px;height:310px;padding:10px 20px"
    ...
    <form id="fms" method="post" novalidate>
        ...
        <div class="fitem">
            <label>Nama Kelurahan:</label>
            <input id="ccinsert3" class="easyui-combobox" name="kodekelurahan" data-options="  
                   valueField: 'kodekelurahan',  
                   textField: 'namakelurahan'" />  
        </div>          
        <div class="fitem">
