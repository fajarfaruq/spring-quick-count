...
<script type="text/javascript">
    var url;
    function newUser() {
        $('#dlg').dialog('open').dialog('setTitle', 'Tambah User Admin');
        $('#fm').form('clear');
        url = '${pageContext.request.contextPath}/user/useradmin/tambah';
    }
    function editUser() {
        var row = $('#dg').datagrid('getSelected');
        if (row) {
            $('#dlgedit').dialog('open').dialog('setTitle', 'Edit User Admin');
            $('#fms').form('load', row);
    ...
    function removeUser() {
        ...
        if (row) {
            $.messager.confirm('Konfirmasi', 'Apakah anda yakin akan menghapus user kelurahan tersebut?', function(r) {
                if (r) {
                    $.post('${pageContext.request.contextPath}/user/useradmin/hapus', {id: row.username}, function(result) {
                        if (result.success) {
                            $('#dg').datagrid('reload');	// reload the user data
                        } else {
    ...
    <div class="manajemendata">
        ...
        <table id="dg" class="easyui-datagrid" 
               url="${pageContext.request.contextPath}/user/useradmin/tampil"
               toolbar="#tb"
...
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">
    <div class="ftitle">Informasi User Admin</div>
